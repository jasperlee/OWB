 ...
 */
  ...
  {
      $sql = "select  * from check_valid where CheckID = '$code' and type=1";
	...
	if(!$result)
      ...
      /*对比加密后的密码md5值*/
      $email = $row["Email"];
      $sql = "select * from unactive_user where Email = '$email'";
      $result = mysql_query($sql);
      $num_rows = mysql_num_rows($result);
      ...
      /*删除原有项*/
      $delete_sql = "delete from unactive_user where Email = '$email'";
  ...
  if($code && active_user($code))  //userName password存在 且能激活
